    ...
    fun calculateBet(
        ...
        val decision = when {
            ...
            hasStrong -> {
                ...
                println("  STRONG HAND -> Raise to $raiseAmount (call $callAmount + 2×minRaise $minimumRaise)")
                raiseAmount
            }
            hasDecent && callAmount <= smallBetThreshold -> {
                println("  DECENT HAND & Small bet ($callAmount <= $smallBetThreshold) -> Call $callAmount")
                callAmount
            }
            hasWeakPlayable && callAmount <= smallBlind * 2 -> {
                println("  WEAK PLAYABLE & Tiny bet ($callAmount <= ${smallBlind * 2}) -> Call $callAmount")
                callAmount
            }
            // Risky play only for marginal non-playable hands
            isRiskMood && hasMarginal && !hasWeakPlayable && callAmount <= pot / 3 -> {
                val riskyPlay = calculateRiskyPlay(callAmount, minimumRaise, myStack)
                println("  RISK MOOD & MARGINAL HAND & Small relative bet ($callAmount <= ${pot/3}) -> Risky play: $riskyPlay")
                riskyPlay
            }
            else -> {
                println("  NO QUALIFYING HAND/SITUATION -> Fold (0)")
                0
            }
        }
